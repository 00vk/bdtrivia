...
1. **Resolve paths** — determine repo root, derive feature-slug, create artifact directory `.artifacts/<feature>/`.
...
4. **Compile the research document** — once all workers return, compile a single `research.md` with sections: title, scope, existing functionality, one section per direction, cross-references, open questions. Include header: `<!-- feature: <slug> | date: <date> | agent: research-lead -->`.
...
6. **Commit** — `git add .artifacts/<feature>/ && git commit -m "<feature>: research phase complete"`.

7. **Output** artifact path and summary. Next step: `/design <feature-slug>`.
